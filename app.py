...
# ─────────────────────────────────────────────
#  Route test image directe
# ─────────────────────────────────────────────
@app.route("/admin/test-images")
@login_required
def admin_test_images():
    """Affiche toutes les URLs stockées en base avec les images en direct."""
    photos = Photo.query.order_by(Photo.created_at.desc()).limit(10).all()
    html = "<h2 style='font-family:sans-serif;padding:1rem'>Test affichage images</h2>"
    html += f"<p style='font-family:sans-serif;padding:0 1rem'>{len(photos)} photo(s) en base</p>"
    for p in photos:
        html += f"""
        <div style='padding:1rem;border-bottom:1px solid #333;font-family:sans-serif'>
            <p><b>ID:</b> {p.id} | <b>Catégorie:</b> {p.category}</p>
            <p><b>URL stockée:</b> <a href='{p.filename}' target='_blank'>{p.filename}</a></p>
            <img src='{p.filename}' style='max-width:300px;max-height:200px;border:2px solid red'
                 onerror="this.nextSibling.style.display='block'"
                 alt='test'>
            <p style='display:none;color:red'>❌ Image non chargée</p>
        </div>"""
    if not photos:
        html += "<p style='padding:1rem;color:red'>Aucune photo en base</p>"
    return html
